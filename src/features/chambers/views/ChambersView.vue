<template>
  <div class="space-y-5">
    ...
    <div v-if="showForm" class="f-card space-y-4">
      <h4
        class="font-extrabold text-[14px]"
    ...
    <!-- Chambers grid -->
    <div
      v-if="store.loading && !store.chambers.length"
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            ...
            <p class="text-[12px] mt-1" style="color: #9aa0b8">
              {{ c.address || "No address" }}
            </p>
          </div>
          <button
            @click="del(c)"
            class="text-[11px] font-bold px-2.5 py-1.5 rounded-lg transition-all"
            style="background: rgba(224, 82, 82, 0.08); color: #e05252"
            @mouseover="
              (e) => (e.currentTarget.style.background = 'rgba(224,82,82,0.15)')
            "
            @mouseout="
              (e) => (e.currentTarget.style.background = 'rgba(224,82,82,0.08)')
            "
          >
            Delete
          </button>
